  ...
  describe "get .get-energy-certificate-data.epb-frontend/jwks" do
    context "when the request received" do
      ...
      it "includes all the required keys" do
        ...
        keys = response_hash["keys"]
        expect(keys.first.keys.sort).to eq(%w[e kid kty n use])
      end

      it "returns the expected data" do
        response_hash = JSON.parse(response.body)
        expect(response_hash["keys"]).to be_an(Array)
        keys = response_hash["keys"]
        expect(keys.first["kty"]).to eq("RSA")
        expect(keys.first["kid"]).to eq("355a5c3d-7a21-4e1e-8ab9-aa14c33d83fb")
        expect(keys.first["use"]).to eq("sig")
        expect(keys.first["e"]).to eq("AQAB")
      end
      ...
      it "returns the expected encoded modulus on 'n' key" do
        ...
        expect(keys.first["n"]).to eq(expected_n)
      end
